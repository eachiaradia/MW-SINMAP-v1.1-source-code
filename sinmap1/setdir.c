    ...
    /*  allocate memory for slope and angle  */
      ...
      printf("MESSAGE: Calculate direction ...\n");
      //printf("PERCENT: %d\n",75);
    ...
    for that recursion level.  
      */
    iter=1;
    printf("MESSAGE: Resolving %d Flats, Iteration: %d \n",n,iter);
    ...
    elev2 = (float *)malloc(sizeof(float) *n);
      ...
      {
         printf("ERROR: Unable to allocate at iteration %d\n",iter);
      }
    /*  Put unresolved pixels on stack  */
    	  ...
    	  for(j=i1; j<n1; j++)
      {
        ...
        if(dir[j][i] == 0)
        {
    	  ...
    	  ip++;
    	  if(ip > n)printf("ERROR: Stack logic\n");
        }
      }
    ...
    /*  The direction 9 was used to flag pits.  Set these to 0  */
      ...
      printf("MESSAGE: Done iteration %d\nMESSAGE: Flats resolved %d\n",iter,n);
    ...
    /*  Put unresolved pixels on new stacks - keeping in same positions  */
        ...
        if(dir[js[sloc[ip]]][is[sloc[ip]]] == 0)
        {
     	  ...
     	  /*   Initialize the stage 1 array for flat routing   */
    		s[sloc[ip]] = 1;
    	  ipp++;
    	  if(ipp > nu)printf("ERROR: Stack logic\n");
    	}
    	else
    	{
    ...
    /*			if 	adjacent to same level or lower that drains or 
    		...
    		st=st+1;
    		//printf("MESSAGE: Incfall %d %d \n",ninc,n);
    		if(ninc == nincold)
    		{
    			done = 1;
    			printf("ERROR: There are pits remaining, direction will not be set\n");
